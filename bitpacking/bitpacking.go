...
// using the specified bits-per-key (bpk).
//
...
func Encode16(input []uint16, bpk int, output []byte) {
	...
	switch bpk {
	case 12:
		encode12bpk(input, output)
	case 16:
		encode16bpk(input, output)
	default:
...
func encode16bpk(input []uint16, output []byte) {
	// Hack to verify if we're little endian.
	// TODO: disable sometimes with invariants on.
	...
	if len(input) >= 8 {
		...
		for i, v := range in4 {
			...
			binary.BigEndian.PutUint64(out8[i][:], x) //gcassert:bce
		}
