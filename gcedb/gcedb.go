...
const (
	...
	// H3 series: Intel Sapphire Rapids (HPC), fixed 88 vCPUs, 352 GiB.
	h3Fixed = 4 // 352 GiB / 88 vCPUs = 4 GiB/vCPU
...
func parseCustomMachineType(family string, parts []string) (MachineInfo, error) {
	...
	case "n1":
		...
		info.AllowedLocalSSDCount = localSSDCountsN1(vcpus)
...
func getPredefinedMachineInfo(family, variant string, vcpus int, hasLSSD, hasMetal bool) (MachineInfo, error) {
...
func localSSDCountsN1(vcpus int) []int {
	// N1: 1-8, 16, 24 depending on vCPU count.
	switch {
	case vcpus <= 10:
